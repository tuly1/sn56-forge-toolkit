...
# production. The older 75a0a20c break-glass commit is intentionally not wired
# here: it is secondary recovery only, never the immediate rollback target.
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPOINT="$SCRIPT_DIR/sn56-week11-repoint.sh"
MANIFEST="$SCRIPT_DIR/../release/week11-release-manifest.json"
EXPECTED_ROLLBACK="59e0698c952edaf1bf34a117ecad41bce87517cf"
