...
#
# USAGE
#   ./sn56-week11-repoint.sh --manifest MANIFEST --contract-only
#   ./sn56-week11-repoint.sh --manifest MANIFEST --dry-run
#   ./sn56-week11-repoint.sh --manifest MANIFEST
#   ./sn56-week11-repoint.sh --manifest MANIFEST --rollback
#
# EXIT CODES
...
# Byte-exact schema-2 policy for the sealed candidate and completed offline
# canary. The Week-11 contract rejects every policy substitution or drift.
...
# Hard abort: Wednesday 2026-08-26 16:30:00 UTC, 90 minutes before the
# 18:00 UTC science deadline. Precomputed to avoid BSD/GNU `date -d`
# divergence between macOS control box and Linux host.
HARD_ABORT_EPOCH=1787761800
HARD_ABORT_HUMAN="2026-08-26 16:30:00 UTC"
...
if [ "$NOW_EPOCH" -ge "$HARD_ABORT_EPOCH" ]; then
  if [ "$MODE" = "rollback" ]; then
    ...
    bad "a forward repoint at/after the Week-11 cutoff leaves insufficient recovery margin before the 18:00 UTC science deadline."
...
if [ "$MODE" = "rollback" ]; then
  ok "target Docker identities are bound to the exact reviewed candidate policy"
else
...
# DONE
# ============================================================================
EV="$EVIDENCE_DIR/SN56-WEEK11-REPOINT-EVIDENCE-$STAMP.json"
