...
# Hard abort: Wednesday 2026-08-26 16:30:00 UTC. This fixed Week-11 release
# cutoff reserves verification and rollback margin. Precomputed to avoid BSD/GNU `date -d`
...
if [ "$NOW_EPOCH" -ge "$HARD_ABORT_EPOCH" ]; then
  if [ "$MODE" = "rollback" ]; then
    ...
    bad "a forward repoint at/after the Week-11 cutoff leaves insufficient verification and rollback margin."
