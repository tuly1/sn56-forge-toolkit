...
def test_toolkit_image_keeps_owned_krea_runtime_structurally_separate():
    contents = TOOLKIT_DOCKERFILE.read_text(encoding="utf-8")

    assert "PYTHONDONTWRITEBYTECODE=1" in contents
    assert "find /app/ai-toolkit /opt/sn56/krea-ai-toolkit -xdev" in contents
    ...
    assert (
        ...
        "c06a2151891f084cc3ebdf8e30408140b4d2b6f6"
    ) in contents


def test_legacy_flux_image_carries_only_incumbent_aitoolkit_and_kohya():
    ...
    assert contents.count("PYTHONDONTWRITEBYTECODE=1") == 2
    assert "find /app/ai-toolkit -xdev -type d -name __pycache__" in contents
    assert (
    ...
    for owned_runtime_surface in (
        ...
        "sn56_krea_runtime_capabilities.json",
        ".sn56-runtime-identity.json",
    ):
        assert owned_runtime_surface not in contents
    assert "--requirement /opt/sn56/image-runtime-lock.txt" in contents
    assert "python3 /opt/sn56/verify-image-runtime.py" in contents
...
def test_image_build_network_access_has_bounded_retries(
    ...
    if dockerfile == TOOLKIT_DOCKERFILE:
        assert "retry_network git fetch --depth=1 origin c06a215" in contents
    else:
        assert "retry_network git fetch --depth=1 origin c06a215" not in contents
...
def test_image_sources_are_pinned_to_certified_identities():
    ...
    assert dockerfile.count("c06a2151891f084cc3ebdf8e30408140b4d2b6f6") == 4
