    ...
    *,
    ...
    if valid:
        ...
        if selection is None:
            # A frozen production target is authoritative. Valid bytes outside
            # its planned schedule are not eligible for legacy selection.
...
def select(
    ...
    if (state or {}).get("checkpoint_target") is not None:
        # Presence of a validated frozen target suppresses every legacy selector,
        # including when no current candidate falls within its planned schedule.
...
def _select_from_frozen_fraction(
    ...
    """Prefer the precomputed target, salvaging nearest current output if absent."""
    ...
    for path in valid:
        ...
        if step <= 0 or step > planned:
            # A malformed/out-of-schedule file must not win a fixed policy, but
            # it also must not discard other valid current-run recovery points.
            continue
    ...
    if exact:
        step, path = min(exact, key=lambda row: os.path.basename(row[1]))
        return Selection(
            path=path,
            source="frozen_checkpoint_fraction",
            reason=(
                "selected the exact current-run checkpoint mapped to the "
                f"frozen curve fraction {numerator}/{denominator}"
            ),
            step=step,
        )

    # Production is never-forfeit: on a slower validator or deadline stop, use
    # the nearest valid current-run checkpoint to the same frozen fraction.
    # If every save is earlier than the target, this naturally chooses the
    # highest available step. Exact distance ties choose the earlier step.
    step, path = min(
        rows,
        key=lambda row: (
